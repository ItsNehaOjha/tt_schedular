...
const TimetableGrid = ({ onBack, onSave, onPublish, data, timetableData, mode, isEditable, showPDFExport = false }) => {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {(isEditable || mode === 'edit' || mode === 'create') && (
              <>
                <button onClick={() => { clearLocalTimetableData(); clearLocalTimetableData() }} className="p-2 hover:bg-white/20 rounded-lg transition-colors" title="Clear Timetable"><RotateCcw className="w-5 h-5" /></button>
            ...
            {onPublish && (isEditable || mode === 'edit' || mode === 'create') && (
              <button onClick={handlePublish} className="px-4 py-2 bg-green-500 hover:bg-green-600 rounded-lg transition-colors flex items-center space-x-2" disabled={!validation.isValid}>
                <Share className="w-4 h-4" /><span>Publish</span>
              </button>
            )}
          </div>
        </div>
